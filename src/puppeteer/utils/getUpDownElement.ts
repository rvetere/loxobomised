...
export const getUpDownElement = async (container: ElementHandle<Node> | null) => {
  ...
  // Selecting awning buttons by their SVG path is not reliable - we will find it in ANY case
  // but after the server is running for more than one hour, the click suddenly stops working - but only on one of two awnings :D
  ...
  // );
  if (!upButtons.length && !downButtons.length) {
    const texts = await container.$$eval("div[role=button]", (divs) =>
      divs.map((div) => div.innerHTML)
    );
    const allButtons = await container.$$("div[role=button]");
    // The order of the buttons is reversed compared to the UI - thanks to flex-box ;-)
    return {
      upButton: allButtons[1],
      downButton: allButtons[2],
    };
  }
  return {
